...
namespace Project_FinchControl
{
    ...
    class Program
    {
            ...
            foreach (Commands command in commands)
            {
                switch(command)
                {
                    ...
                    case Commands.GETLIGHTLEVELRIGHT:
                        commandOut = $"Right Light Sensor value: {finchRobot.getRightLightSensor().ToString("n2")}\n";
                        break;
                    case Commands.GETLIGHTLEVELLEFT:
                        commandOut = $"Left Light Sensor value: {finchRobot.getLeftLightSensor().ToString("n2")}\n";
                        break;
                    case Commands.GETLIGHTLEVELAVERAGE:
                        int lightAvg;
                        lightAvg = ((finchRobot.getLeftLightSensor() + finchRobot.getRightLightSensor()) / 2);
                        commandOut = $"Light Level Average for both Light Sensors = {lightAvg.ToString("n2")}\n";
                        break;
                    case Commands.NOTEOFF:
                        finchRobot.noteOff();
                        commandOut = Commands.NOTEOFF.ToString();
                        break;
                    case Commands.NOTEON:
                        finchRobot.noteOn(400);
                        commandOut = Commands.NOTEON.ToString();
                        break;
                    case Commands.NONE:
                        break;
                    case Commands.DONE:
                        commandOut = Commands.DONE.ToString();
                        break;
                    default:
                        break;
                }
        ...
        {
            DisplayScreenHeader("All of the Finch Commands Available");
            foreach(Commands command in commands)
            {
                Console.WriteLine($"\t{command}");
            }
            DisplayMenuPrompt("User Programming return to Menu");
        }
        ...
        static void UserProgrammingDisplayGetFinchCommands(List<Commands> commands)
        {
            ...
            int numOfCommand = 1;
            foreach (string nameCommands in Enum.GetNames(typeof(Commands)))
            {
                Console.Write($"-{nameCommands.ToLower()} -\n");
                if (numOfCommand == 0) Console.Write("-\n\t-");
                numOfCommand++;
            }
            while(command != Commands.DONE)
            {
                ...
                {
                    Console.WriteLine("Invalid entry, please enter a valid input.");
                }
            }
            DisplayMenuPrompt("User Programming return to Menu");
        }
        ...
        static (int motorSpeed, int ledBrightness, double waitSeconds) UserProgrammingDisplayGetCommandParameters()
        {
            ...
            (int motorSpeed, int ledBrightness, double waitSeconds) commandParam;
            commandParam.ledBrightness = 0;
            commandParam.motorSpeed = 0;
            commandParam.waitSeconds = 0;

            Console.WriteLine("Enter the motor speed you wish to have, between 1 - 255");
            int.TryParse(Console.ReadLine(), out commandParam.motorSpeed);
            while (commandParam.motorSpeed < 0 || commandParam.motorSpeed >= 255 || commandParam.motorSpeed == 0) ;
            {
                Console.WriteLine("Invalid value, please try again.");
                commandParam.motorSpeed = int.Parse(Console.ReadLine());
            }
            Console.WriteLine("Value {0} is valid for motorspeed", commandParam.motorSpeed);
            Console.WriteLine();
            Console.WriteLine("Enter the time you wish to have in seconds, between 1 - 10");
            double.TryParse(Console.ReadLine(), out commandParam.waitSeconds);
            while (commandParam.waitSeconds < 0 || commandParam.waitSeconds >= 10 || commandParam.waitSeconds == 0) ;
            {
                Console.WriteLine("Invalid value, please try again.");
                commandParam.waitSeconds = double.Parse(Console.ReadLine());
            }
            Console.WriteLine("Value {0} is valid for time in seconds", commandParam.waitSeconds);
            Console.WriteLine();
            Console.WriteLine("Enter the led brightness you wish to have, between 1 - 255");
            int.TryParse(Console.ReadLine(), out commandParam.ledBrightness);
            while (commandParam.ledBrightness < 0 || commandParam.ledBrightness >= 255 || commandParam.ledBrightness == 0) ;
            {
                Console.WriteLine("Invalid value, please try again.");
                commandParam.ledBrightness = int.Parse(Console.ReadLine());
            }
            Console.WriteLine("Value {0} is valid for led brightness", commandParam.ledBrightness);
            Console.WriteLine();
            Console.WriteLine($"\tTime requested: {commandParam.waitSeconds}");
            Console.WriteLine($"\tBrightness Requested: {commandParam.ledBrightness}");
            Console.WriteLine($"\tSpeed Requested: {commandParam.motorSpeed}");
            Console.WriteLine();
            DisplayMenuPrompt("User Programming Menu Return");
            return commandParam;
        }
        #endregion
